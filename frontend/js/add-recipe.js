...
function showGateMessage() {
  ...
  const login = document.createElement('a'); login.href = './login.html'; login.textContent = 'Login';
  const sep = document.createTextNode(' | ');
  const reg = document.createElement('a'); reg.href = './register.html'; reg.textContent = 'Register';
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      await fetchJSON('/api/recipes', {
        ...
        body: JSON.stringify({ name, category, cookTime, servings, source, ingredients, instructions })
      });
